...
class UptimeTickService {
    ...
    public async findMostRecentTick(): Promise<UptimeTick> {
        return await this.uptimeTicks.findOne().sort('-score');
    }

    public async createUptimeTick(uptimeTickData: CreateUptimeTickDto): Promise<UptimeTick> {
    ...
    public async deleteAll(thresholdDate: Date): Promise<Number> {
        const deletionOutcome: any = await this.uptimeTicks.deleteMany({ datetime: { $lte: thresholdDate } });
